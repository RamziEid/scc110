...
public class Ball
{
     ...
     * @param x The x co-ordinate of a centre of the Ball (in pixels)
    ...
    public boolean lineContact(Line l) {
        double distance = Math.abs(l.getXStart() - xPosition);

        if (distance > size / 2) {
            return false;
        } else {
            double minY = Math.min(l.getYStart(), l.getYEnd());
            double maxY = Math.max(l.getYStart(), l.getYEnd());

            if (yPosition + size / 2 < minY || yPosition - size / 2 > maxY) {
                return false;
            } else {
                return true;
            }
